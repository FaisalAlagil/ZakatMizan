...
export function InvestmentScreeningInput({
  ...
  const [holdings, setHoldings] = useState<InvestmentHolding[]>([
    {
      id: 'inv-1',
      name: 'Investment 1 (e.g. SPUS, AAPL, TFSA)',
      ticker: '',
  ...
  function updateHolding(id: string, patch: Partial<InvestmentHolding>) {
    const next = holdings.map((h) => {
      ...
      if ('annualDividend' in patch || 'halalRatio' in patch || 'shariahStatus' in patch) {
  ...
  const portfolio = calculatePortfolioPurification(holdings)
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between border-b border-hair pb-2.5">
        <span className="text-xs font-medium text-ink">
          Investment &amp; Equity Holdings ({holdings.length})
        </span>
          ...
          >
            <div className="flex items-center justify-between gap-2">
              <input
                ...
                placeholder={`e.g. SPUS, Apple (AAPL), S&P 500 ETF, TFSA Portfolio`}
            ...
            <div className="grid grid-cols-12 gap-2 items-center">
              ...
              <div className="col-span-6 flex items-center border border-hair rounded-xl px-2.5 py-1.5 bg-canvas focus-within:border-deep">
                ...
                <input
                  ...
                  placeholder="0"
                  aria-label={`${h.name} market value`}
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    updateHolding(h.id, {
                      ...
                      halalRatio,
                      purificationRatio: Math.max(0, 1 - halalRatio),
                      purificationDue: (h.annualDividend || 0) * (1 - halalRatio),
                    })
                  }}
      ...
      <div className="rounded-2xl border border-hair bg-paper p-4 text-center space-y-1.5 shadow-sm">
        <div className="flex items-baseline justify-center gap-2">
          ...
          <span className="display text-2xl font-semibold text-ink">
            {money(portfolio.totalMarketValue || Number.parseFloat(value) || 0, currency, 2)}
          </span>
        </div>

        {portfolio.totalPurificationDue > 0 && (
          <div className="rounded-xl bg-haram/5 border border-haram/20 p-2 text-xs text-haram">
            <strong>{money(portfolio.totalPurificationDue, currency, 2)}</strong> dividend purification must be separated &amp; donated to charity.
          </div>
        )}
      </div>
    </div>
